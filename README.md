...
It is not meant for real-time triage as bidirectional sampling means seeds see their causal future. 
  ...
  Beyond a PoC, for incident automation in Sentinel, the use-case and validity of the bidirectional sampling should be tested against real data to verify that the chain-starting event and its timestamp (and an indicative timespan of the incident), provided by the SIEM during incident creation, led to a correct identification of kill chain events. This requires the KQL side to be highly optimized, striving for completeness of data necessary for the graph at the lowest computational effort possible with in-built safeguards for node explosions.  
